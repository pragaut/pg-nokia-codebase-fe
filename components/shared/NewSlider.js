...
const ItemWrapper = styled.div`
    ...
    &:first-child{
        margin-left:${props => props.stateIndex === 0 ? (props.marginleft ? props.marginleft : "18px") : (props.marginleft ? props.marginleft : "0px")}
    }
`;
...
class NewSlider extends Component {
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        if (nextProps.visibleItems != this.state.visibleItems) {
            this.setState({
                currentIndex: nextProps.activeItemIndex,
                leftArrowImage: 'arrowleftgrey.png',
                rightArrowImage: 'arrowright.png',
                visibleItems: nextProps.visibleItems,
                showArrows: false, //(nextProps.visibleItems < this.props.children.length) && (nextProps.visibleItems > 1)
            });
        }
    }
    UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    componentDidMount() {
        ...
        this.setState({
            ...
            leftArrowImage: 'arrowleftgrey.png',
            rightArrowImage: 'arrowright.png',
            showArrows: false, // (this.props.visibleItems < this.props.children.length) && (this.props.visibleItems > 1)
        });
    }
    ...
    next = () => {
        const currentIndex = Math.min(this.props.children.length - this.props.visibleItems, this.state.currentIndex + 1);
                ...
                <Wrapper id='wrapper' renderedCount={this._timesRendered} browser={this.props.browser} visibleItems={this.state.visibleItems}>
                    ...
                    <CarouselContainer id='container' >
                        {children && children.map((child, index) => (
                            <ItemWrapper Transform={this.props.Transform} index={index} previousIndex={this._previousIndex} stateIndex={this.state.currentIndex} headerColumnIncluded={this.props.headerColumnIncluded} key={index} visibleItems={this.state.visibleItems} bgColor={this.props.bgColor} marginleft={this.props.marginleft} >
                                <CarouselSlot data={this.state.translateValue}>
                                    <Item
                                        ...
                                        index={index} previousIndex={this._previousIndex} headerColumnIncluded={this.props.headerColumnIncluded} stateIndex={this.state.currentIndex} id={index}>{child}</Item>
                                </CarouselSlot>
                            </ItemWrapper>
                        ))}
                    </CarouselContainer>
